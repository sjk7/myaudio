        ...
        ERROR_SUCCESS)
    {
        ...
        while (cr == ERROR_SUCCESS && !found)
        {
            ...
            if (cr == ERROR_SUCCESS)
            {
                ...
                {
                    if ((cr = RegOpenKeyEx(hkEnum, (LPCTSTR)databuf, 0,
                                           KEY_READ, &hksub)) == ERROR_SUCCESS)
                    {
...
static LPASIODRVSTRUCT newDrvStruct(HKEY hkey, char *keyname, int drvID,
                                    LPASIODRVSTRUCT lpdrv)
{
    ...
    if (!lpdrv)
    {
            ...
            ERROR_SUCCESS)
        {
            ...
            datatype = REG_SZ;
            datasize = 256;
            cr = RegQueryValueEx(hksub, COM_CLSID, 0, &datatype,
            ...
            if (cr == ERROR_SUCCESS)
            {
                ...
                if (rc == 0)
                {
                    ...
                    if (lpdrv)
                    {
                        ...
                        lpdrv->drvID = drvID;
                        MultiByteToWideChar(CP_ACP, 0, (LPCSTR)databuf, -1,
                                            (LPWSTR)wData, 100);
                        if ((cr = CLSIDFromString((LPOLESTR)wData,
                                                  (LPCLSID)&clsid)) == S_OK)
                        {
...
AsioDriverList::AsioDriverList()
{
    ...
    {
        if ((cr = RegEnumKey(hkEnum, index++, (LPTSTR)keyname,
                             MAXDRVNAMELEN)) == ERROR_SUCCESS)
        {
...
LONG AsioDriverList::asioGetNumDev(VOID) { return (LONG)numdrv; }
...
LONG AsioDriverList::asioOpenDriver(int drvID, LPVOID *asiodrv)
{
    ...
    if ((lpdrv = getDrvStruct(drvID, lpdrvlist)) != 0)
    {
        if (!lpdrv->asiodrv)
        {
            rc = CoCreateInstance(lpdrv->clsid, 0, CLSCTX_INPROC_SERVER,
                                  lpdrv->clsid, asiodrv);
            if (rc == S_OK)
            {
...
LONG AsioDriverList::asioGetDriverName(int drvID, char *drvname,
                                       int drvnamesize)
{
    LPASIODRVSTRUCT lpdrv = 0;
...
LONG AsioDriverList::asioGetDriverPath(int drvID, char *dllpath,
                                       int dllpathsize)
{
    LPASIODRVSTRUCT lpdrv = 0;
